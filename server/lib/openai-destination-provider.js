...
function destinationPrompt(destinationName, trip) {
  return [
    ...
    "Every destination type (including major cities) sits within a broader travel region. If the trip is 4+ days, include a compact regionalDestinationProfile (primaryDestination, gatewayTowns, metroOrTourismRegion, nearbyDayTrips, overnightExtensions, regionalConfidence) naming real, well-known nearby destinations worth a day trip or a one-night extension within roughly a 4-hour drive -- for example a major city's nearest mountains, coast, or a famous regional draw, not just in-city options. For mountain, national-park, lake, coastal, island, wine-country, or resort destinations specifically, also name real scenic corridors and trail/waterfall places instead of using the park name as one generic activity. Do not silently add a second overnight base -- extensions are optional information for the traveler to approve.",
