...
const providerStatusApi = fs.readFileSync("api/lib/provider-health-actions.js", "utf8");
...
assert.ok(sw.includes("routemosaic-public-v50"));
...
assert.ok(plannerApi.includes("handlePlannerAction"));
assert.ok(plannerActions.includes("validatePlanningProviders"));
assert.ok(plannerActions.includes("PROVIDER_CONFIGURATION_REQUIRED"));
assert.ok(plannerActions.includes("mockDestinationResearch"));
assert.ok(!plannerActions.includes("details: errors"));
